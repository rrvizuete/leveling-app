...
def sort_df_by_point(df: pd.DataFrame, column: str) -> pd.DataFrame:
    if df.empty:
        return df
    work = df.copy()
...
def classify_section(start_point, end_point, start_fixed, end_fixed, component_adjustable):
...
def trace_sections_for_component(component_id, component_points, component_edges, fixed_points_in_component):
    graph = build_graph(component_edges)
            ...
            section_rows.append(
                {
                    ...
                    "Section_Type": classify_section(start_node, end_node, start_fixed, end_fixed, component_adjustable),
...
def run_component_adjustment(component_points, component_cleaned_df, fixed_points_in_component):
    adjusted_elevations = {}
    point_sigmas = {}
    residual_rows = []
    ...
    try:
        ...
        return adjusted_elevations, point_sigmas, residual_rows, "Adjustment matrix is singular for this connected component."
...
def run_network_adjustment(cleaned_df: pd.DataFrame, control_df: pd.DataFrame):
    errors = []
    warnings = []
    ...
    components = build_components(edge_pairs)
    ...
    for idx, component in enumerate(components, start=1):
        ...
        status = "Adjustable" if fixed_in_component else "Not Adjustable"
    ...
    adjusted_points_df = sort_df_by_point(adjusted_points_df, "Point_ID")
    ...
    control_checks_df = sort_df_by_point(control_checks_df, "Point_ID")
